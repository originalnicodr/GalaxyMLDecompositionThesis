...
def analyze_galaxy_2_clusters_linkages(
    ...
    gal, cut_idxs = remove_outliers(gal)
...
def get_star_cut_indexes(sdf, cut_radius_factor):
...
def remove_outliers(gal, *, cut_radius_factor=3):
    
    # convertimos las estrellas en un dataframe
    sdf = gal.stars.to_dataframe()
    
    # nos fijamos que filas hay que borrar y cual radio de corte nos da
    cut_idxs, cut_radius = get_star_cut_indexes(sdf, cut_radius_factor=cut_radius_factor)
