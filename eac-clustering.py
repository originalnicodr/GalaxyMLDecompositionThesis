...
def analyze_galaxy_n_clusters_linkages(
    ...
    for i in range(0, 100):
        estimators.append(KMeans(n_clusters=100))
        gc.collect()
    ...
    print("zeros: ", np.count_nonzero(labels == 0))
    print("ones: ", np.count_nonzero(labels == 1))
    print("nans: ", np.count_nonzero(np.isnan(labels)))
    ...
    with open(results_path+'/'+gal_name+'/' + 'internal_evaluation.csv', 'a') as f:
        ...
        f.write(f"eac,Silhouette,{s_score}\n")
        f.write(f"eac,Davies Bouldin,{db_score}\n")
